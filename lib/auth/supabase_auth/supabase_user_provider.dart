...
class PowerSyncTemplateSupabaseUser extends BaseAuthUser {
  PowerSyncTemplateSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> powerSyncTemplateSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PowerSyncTemplateSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
